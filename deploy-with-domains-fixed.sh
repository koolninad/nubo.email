...
# Nubo.email Production Deployment with Proper Domains (Fixed)
# Frontend: nubo.email
...
# Create nginx config for initial HTTP (for Let's Encrypt)
...
http {
    ...
    # HTTP only initially for Let's Encrypt
    server {
        listen 80;
...
# Start services with HTTP nginx
...
echo -e "${GREEN}Updating nginx configuration with SSL...${NC}"
...
(crontab -l 2>/dev/null | grep -v "certbot renew"; echo "0 0,12 * * * docker compose -f ${DEPLOYMENT_DIR}/docker-compose.yml run --rm certbot renew --quiet && docker compose -f ${DEPLOYMENT_DIR}/docker-compose.yml exec nginx nginx -s reload") | crontab -
